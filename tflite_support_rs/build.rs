...
fn prepare_tensorflow_source(tf_src_path: &Path) {
    ...
    if !complete_clone_hint_file.exists() {
        ...
        let bazel_build_path = root.join("build-res/shared_objects.bzl");

        let bindgen_path = tf_src_path.join("tensorflow_lite_support/c/bindgen");
        std::fs::create_dir_all(bindgen_path.clone()).unwrap();
        std::fs::copy(bazel_build_path, bindgen_path.join("BUILD"));
    }
}
...
fn lib_output_path() -> PathBuf {
    ...
    out_dir().join(format!("{}tflite_support_c.{}", lib_prefix, ext))
}
...
fn build_tensorflow_with_bazel(tf_src_path: &str, config: &str) {
    ...
    let libc_out_dir = PathBuf::from(tf_src_path)
        ...
        .join("c")
        .join("bindgen");
    // let libcc_out_dir = PathBuf::from(tf_src_path)
    //     .join("bazel-bin")
    //     .join("tensorflow_lite_support")
    //     .join("cc")
    //     .join("task")
    ...
    let obj_detector_basename = format!("{}object_detector_c.{}", lib_prefix, ext);
    let bazel_clib_output_path_buf = libc_out_dir.join(obj_detector_basename.clone());

    let bazel_target = "//tensorflow_lite_support/c/bindgen:object_detector_c";
    ...
    let mut bazel = std::process::Command::new("bazel");
    {
        ...
        // let bazel_output_base_path = out_dir().join(format!("tflite_support_{}_output_base", TAG));
        // bazel.arg(format!(
        //     "--output_base={}",
    ...
    copy_or_overwrite(&bazel_clib_output_path_buf, &c_lib_out);
}
